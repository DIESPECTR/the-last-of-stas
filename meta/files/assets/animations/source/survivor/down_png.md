---
SECTION_ID: files.assets.animations.source.survivor.down_png
TYPE: file/image
---

# Survivor Master — Facing Down (Photoreal)

FILE: assets/animations/source/survivor/down.png
UTILITY: gpt_image
WIDTH: 1024
HEIGHT: 1024
DESCALE: 2
...
USAGE: Static animation master. Fed to `wan` (image-to-video) for idle/walk/attack down-facing clips, then sliced into the 128×128 4×4 sprite sheet. Must stay a clean flat cutout — game lighting (rim-light, shadow, bloom) is applied by the engine at render time, not baked in here.
PROMPT: |
  ...
  weathered face, practical dark trousers.
  ...
  Subject: the same survivor, standing in a relaxed idle stance, facing directly down toward the camera
  (top of head barely visible, mostly seeing the top of shoulders/chest/face from above), completely
  unarmed — empty hands, no weapon, no prop, no gun, no tool.
...
- This master feeds `wan` for idle/walk/attack animation and must survive rembg keying cleanly — flat
  light and pure white background are non-negotiable, even though the reference is dramatically lit.
- Photoreal material detail should be dense enough to read at 128px after downscale — favour clear large
  shapes (coat silhouette, backpack, shoulder guard) over fine micro-detail that will disappear.
- Companion masters: up.png, left.png, right.png — same character, same lighting rules, different facing.
