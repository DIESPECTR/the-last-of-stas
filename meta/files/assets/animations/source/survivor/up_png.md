---
SECTION_ID: files.assets.animations.source.survivor.up_png
TYPE: file/image
---

# Survivor Master — Facing Up (Photoreal)

FILE: assets/animations/source/survivor/up.png
UTILITY: gpt_image
WIDTH: 1024
HEIGHT: 1024
DESCALE: 2
...
USAGE: Static animation master. Fed to `wan` (image-to-video) for idle/walk/attack up-facing clips, then sliced into the 128×128 4×4 sprite sheet. Must stay a clean flat cutout — game lighting (rim-light, shadow, bloom) is applied by the engine at render time, not baked in here.
PROMPT: |
  ...
  Subject: the same survivor, standing in a relaxed idle stance, facing directly away from the camera
  (back of head, back of coat, backpack fully visible, walking away framing), completely unarmed —
  empty hands, no weapon, no prop, no gun, no tool.
...
- Same identity/proportions/scale as down.png — only facing changes, so the four directions read as one
  consistent character in-engine.
