---
SECTION_ID: files.assets.animations.source.runner.up_png
TYPE: file/image
---

# Runner Master — Facing Up (Photoreal)

FILE: assets/animations/source/runner/up.png
UTILITY: gpt_image
WIDTH: 1024
HEIGHT: 1024
DESCALE: 2
...
USAGE: Static animation master. Fed to `wan` (image-to-video) for idle/walk/attack up-facing (away from camera) clips, then sliced into the 128×128 4×4 sprite sheet.
PROMPT: |
  ...
  one exposed wrapped/bandaged ankle. Compact, angular, visibly smaller and faster-looking silhouette.
  Tiny sickly yellow-green accents restricted to eyes and fingertips only. No weapon, no prop, no tool.
  Pose: aggressive forward-leaning sprint stance, seen from directly behind/above, sprinting away from
  camera — back of hood, hunched shoulders, trailing clawed hand visible.
...
- Must match down/left/right in identity/proportions/scale — only facing changes.
- Photoreal replacement of the old charcoal-sketch Runner.
